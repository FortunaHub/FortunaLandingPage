  ...
  overview: (
    <div className="space-y-6 text-sm text-white/68">
      <p>
        ...
        evidence, attack paths, identity and RBAC context, runtime network visibility, runtime signals, and one user-facing risk
      ...
      <Section title="Current product surfaces">
        <ul className="ml-2 list-inside list-disc space-y-2">
          ...
          <li><strong className="text-white/72">Pipeline & Runtime Health</strong> (<code className="text-fortuna-pink">/#/monitoring</code>): agent sync, CVE/SBOM processing, runtime visibility, and quiet/unavailable states.</li>
        </ul>
      </Section>
      ...
      <Section title="What Fortuna correlates">
        <ul className="ml-2 list-inside list-disc space-y-2">
          ...
          <li>runtime/runtime events and network observations when the optional runtime stack is enabled.</li>
...
kubectl -n fortuna rollout status deploy/fortuna-dashboard`}</CodeBlock>
      ...
      <Section title="Environment baseline">
        ...
        <ul className="ml-2 list-inside list-disc space-y-1 text-xs">
          ...
          <li>Agent requires runtime socket access for image/SBOM inspection; runtime/runtime paths increase memory needs.</li>
        </ul>
      </Section>

      <Section title="Local rebuild pipeline">
        <p className="mb-3">
          For lab development, the product repo keeps a full rebuild/deploy pipeline. Add runtime only when runtime/runtime events are
...
./scripts/pipeline/full-clean-database-rebuild-deploy.sh --full --db-reset --with-runtime`}</CodeBlock>
      ...
      <Section title="Add a remote cluster">
        <p className="mb-3">
          In multi-cluster mode the management cluster runs Core, Dashboard, PostgreSQL, NATS, Agent, and optional runtime. A remote
          cluster runs Agent and optional runtime only, pointed at the management Core external service.
        </p>
